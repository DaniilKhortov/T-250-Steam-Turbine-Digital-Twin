...
/*document.addEventListener('DOMContentLoaded', () => {
    ...
    toggleBtn.addEventListener('click', () => {
        resizablePanel.classList.toggle('open');
    });

    // Додаємо обробник для скидання
...
function calculate() {
        ...
        function displayResults(results) {
            ...
            const resultsG0 = document.getElementById("G0");
            const resultsp0 = document.getElementById("p0");
            const resultst0 = document.getElementById("t0");
            const resultst_tm = document.getElementById("t_tm");

            const resultsN_tp = document.getElementById("N_tp");
            const resultst1 = document.getElementById("t1");
            const resultsG_psg2 = document.getElementById("G_psg2");
            const resultst_psg2 = document.getElementById("t_psg2");

            const resultsG_bypass = document.getElementById("G_bypass");
            const resultspt = document.getElementById("pt");
            const resultsNe = document.getElementById("Ne");
            const resultsBt = document.getElementById("Bt");
            ...
            resultsG0.innerHTML = `${results.qo.toFixed(2)} Гкал/год`;
            resultsp0.innerHTML = `Ih`;
            resultst0.innerHTML = `Ag`;
            resultst_tm.innerHTML = `${results.Qt.toFixed(2)} Гкал/год`;

            resultsN_tp.innerHTML = `${results.Ntp.toFixed(2)} МВт`;
            resultst1.innerHTML = `${results.t1.toFixed(2)} °C`;
            resultsG_psg2.innerHTML = `${results.G_psg2.toFixed(2)} м³/год`;
            resultst_psg2.innerHTML = `${results.t_psg2.toFixed(2)} °C`;

            resultsG_bypass.innerHTML = `${results.G_bypass.toFixed(2)} м³/год`;
            resultspt.innerHTML = `${results.pt.toFixed(2)} МВт`;
            resultsNe.innerHTML = `${results.Ne.toFixed(2)} МВт`;
            resultsBt.innerHTML = `${results.Vt} тис. м³/год`;

           /* resultsDiv.innerHTML = `
                <div><strong>Фактична витрата пари Вт:</strong> ${results.Vt} тис. м³/год</div>
                <div><strong>Теплова потужність Qt:</strong> ${results.Qt.toFixed(2)} Гкал/год</div>
                <div><strong>Кінцева температура t1:</strong> ${results.t1.toFixed(2)} °C</div>
                <div><strong>Витрата через байпасний контур Gбайпас:</strong> ${results.G_bypass.toFixed(2)} м³/год</div>

                <div><strong>Витрата через основний контур Gпсг2:</strong> ${results.G_psg2.toFixed(2)} м³/год</div>
                <div><strong>Температура Tпсг2:</strong> ${results.t_psg2.toFixed(2)} °C</div>
                <div><strong>Потужність Pt:</strong> ${results.pt.toFixed(2)} МВт</div>
                <div><strong>Тепловий вивід Q0:</strong> ${results.qo.toFixed(2)} Гкал/год</div>
                
                <div><strong>Загальна потужність Ne:</strong> ${results.Ne.toFixed(2)} МВт</div>
                <div><strong>"Чиста" потужність Ntp:</strong> ${results.Ntp.toFixed(2)} МВт</div>
            `;*/
        }      
/*
